...
let addressesapi = [
	...
	'https://raw.githubusercontent.com/Dailywuxi/bestIP/main/my.txt', //自建IP
...
let CMproxyIPs = [
	'proxyip.aliyun.fxxk.dedyn.io:HKG',//匹配节点名, 有HK就分配该ProxyIP域名
]
	...
	async fetch (request, env) {
		...
		*/
		...
		if (mytoken.length > 0 && mytoken.some(token => url.pathname.includes(token))) {
			...
			path = env.PATH || "/?proxyip=proxyip.aliyun.fxxk.dedyn.io";
			sni = env.SNI || host;
			type = env.TYPE || type;
...
function surge(content, url) {
	...
	for (let x of 每行内容) {
		if (x.includes('= trojan,')) {
			...
			const 正确内容 = `skip-cert-verify=true, ws=true, ws-path=/?proxyip=proxyip.aliyun.fxxk.dedyn.io, ws-headers=Host:"${host}", tfo=false, udp-relay=false`;
